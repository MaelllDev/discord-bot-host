      ...
      <div className="flex flex-wrap items-center gap-2">
        {!deployment.finished ? (
          <>
            ...
            <Badge tone="amber">publicando…</Badge>
          </>
        ) : failed ? (
          <Badge tone="red">
            <IconAlert className="h-3 w-3" /> falhou
          </Badge>
        ) : (
          <Badge tone="green">
            <IconCheck className="h-3 w-3" /> publicado com sucesso
          </Badge>
        )}
      </div>
      ...
      {showPhases ? (
        <DeployProgress
          ...
          extra={
            failed ? (
              <Alert tone="amber">
                A versão anterior <strong>continua ativa</strong>: o release novo só passa a valer quando a instalação e a
                ativação terminam com sucesso. Corrija o erro e envie novamente — nada de <code>/data</code> foi perdido.
              </Alert>
            ) : null
          }
        />
